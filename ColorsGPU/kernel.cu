...
__device__ int get_num_alive_neighbors(int x, int y, int neighborhood_type, int range, int *board) {
...
__device__ int get_sum_neighbors(int x, int y, int neighborhood_type, int range, int * board) {
    int check_x, check_y, count = 0;
...
__global__ void cuda_move_board_up(int *board, int *board_buffer) {
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x + CELL_WIDTH;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
        int board_val = board[index];
        board_buffer[index - CELL_WIDTH] = board_val + (board_val > 0) - (board_val < 0);
        index += blockDim.x * gridDim.x;
    }
}

//updates the board with the 1D rules
__global__ void cuda_update_board_1D(int *board, int *board_buffer, int *OneD_rules, int OneD_width, int alive_offset) {
...
void call_cuda_UB1D(int *board, int *board_buffer, int *OneD_rules, int OneD_width, int alive_offset) {
    cuda_move_board_up<<<512, 128>>>(board, board_buffer);
    cuda_update_board_1D<<<4, 128>>>(board, board_buffer, OneD_rules, OneD_width, alive_offset);
}


//update the board for hodge rules
__global__ void cuda_update_board_hodge(int* board, int* board_buffer, int * hodge_rules) {
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
        int j = index / CELL_WIDTH;
        int i = index % CELL_WIDTH;

        //if healthy
        if (board[index] == 0) {
            if (get_sum_neighbors(i, j, hodge_rules[4], hodge_rules[5], board) > 1) {
        ...
        //infected
            ...
            int sum = get_sum_neighbors(i, j, hodge_rules[4], hodge_rules[5], board);
...
void call_cuda_UBH(int *board, int *board_buffer, int *hodge_rules) {
    cuda_update_board_hodge<<<512, 128>>>(board, board_buffer, hodge_rules);
}


//update the board with nondeterministic rules
__global__ void cuda_update_board_non_deterministic(int* board, int* board_buffer, float *born, float *stay_alive, float *rand_nums) {
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
        ...
        int neighbors = get_num_alive_neighbors(i, j, MOORE, 1, board);
...
void call_cuda_UBND(int *board, int *board_buffer, float *born, float *stay_alive, float *rand_nums) {
    cuda_update_board_non_deterministic<<<512, 128>>>(board, board_buffer, born, stay_alive, rand_nums);
}


//update the board with normal rules. Also includes faders since normal lifelike is just faders with no refractory states
__global__ void cuda_update_board_normal(int* board, int* board_buffer, float *born, float *stay_alive, int num_faders) {
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
...
void call_cuda_UBN(int *board, int *board_buffer, float *born, float *stay_alive, int num_faders) {
    cuda_update_board_normal<<<512, 128>>>(board, board_buffer, born, stay_alive, num_faders);
}


//update the board accordsing to the larger than life ruleset
__global__ void cuda_update_board_LtL(int *board, int *board_buffer, int *LtL_rules, float random_num) {
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
        ...
        int neighbors = get_num_alive_neighbors(i, j, LtL_rules[5], LtL_rules[0], board);
        //if dead
#if reallylowdivergence
...
void call_cuda_UBLtL(int *board, int *board_buffer, int *LtL_rules, float random_num) {
    cuda_update_board_LtL<<<512, 128>>>(board, board_buffer, LtL_rules, random_num);
}
...
void call_cuda_UBS(float *board, float *board_buffer, float *smooth_rules, float r_a_2, float r_i_2, float r_a_2_m_b, float r_i_2_m_b, float r_a_2_p_b, float r_i_2_p_b) {
    cuda_update_board_smooth<<<512, 128>>>(board, board_buffer, smooth_rules, r_a_2, r_i_2, r_a_2_m_b, r_i_2_m_b, r_a_2_p_b, r_i_2_p_b);
}
...
__global__ void cuda_update_smooth_colors(float *board_float, int *board, int *board_buffer, float death_cutoff) {
    //iterate over every cell*/
    unsigned int index = blockIdx.x * blockDim.x + threadIdx.x;
    while (index < CELL_HEIGHT * CELL_WIDTH) {
#if reallylowdivergence
...
void call_cuda_USC(float *board_float, int *board, int *board_buffer, float death_cutoff) {
    cuda_update_smooth_colors<<<512, 128>>>(board_float, board, board_buffer, death_cutoff);
}